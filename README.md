...
**Stack:** Next.js 15 (App Router) · TypeScript strict · Tailwind CSS · Firebase Admin SDK · Vitest · Zod
...
Browser (Next.js client)
  ...
  │     ├── useExitPlanData  - fetches /api/exit-plan, polls every 5 min
  │     └── useExitAnimation - RAF loop, play/pause/scrub state
...
Next.js API routes (Cloud Run)
  ...
  └── POST /api/agent/batch-tick  - Cloud Scheduler target: nudge decisions for all active users
...
lib/
  ...
  ├── remoteConfig.ts  - Firebase Remote Config wrapper with 5-min cache and fallback defaults
...
| Firestore | User docs (`matchStartedAt`, seat, prefs) + nudges collection; row-level security rules and composite indexes deployed via Firebase CLI |
...
| Firebase Remote Config | Nudge cooldown and surge threshold configurable at runtime, no redeploy needed |
...
```
users/{uid}
  matchStartedAt  timestamp   when this user's match clock started
  seat            object      { section, row, number }
  preferences     object      { dietary: string[], avoidCrowds: boolean }
  lastSeenAt      timestamp   updated on every visit; used by batch-tick to filter active users
  createdAt       timestamp

nudges/{nudgeId}
  userId          string
  message         string      nudge text shown to the user
  reasoning       string      Gemini's explanation, kept for debugging
  elapsedMinutes  number      match-minute when the nudge was decided
  stateSnapshot   object      venue wait times at decision time
  createdAt       timestamp
  read            boolean     toggled when the user dismisses the toast
```
...
The curves in `venues/timeline.json` were generated by `scripts/generate-timeline.ts`. Run it once to produce curves for a different venue shape.

**Anonymous auth caveat:** clearing browser cookies starts a new session with a fresh match clock. Seat and preference defaults are restored, but the previous session's nudge history is lost.
...
The repo includes a `Dockerfile` for a multi-stage standalone build. Cloud Run's source-based deploy picks it up automatically; you can also build and push the image manually if needed.
